...
export interface ToolsPageItem {
  id: string;
  name: string;
  desc: string;
  href: string;
...
function getToolIcon(iconName: string): React.ElementType {
  const fromMap = iconMap[iconName.toLowerCase()];
  if (fromMap) return fromMap;
  const lucide = (LucideIcons as Record<string, React.ElementType>)[iconName];
  return lucide || Calculator;
}

const CATEGORY_COLORS: Record<string, string> = {
  "international-trade": "#0F4C81",
  "ocean-freight": "#0F4C81",
  "air-freight": "#8B5CF6",
  "road-rail": "#EA580C",
  "customs-compliance": "#059669",
  warehousing: "#EA580C",
  ecommerce: "#EC4899",
  insurance: "#14B8A6",
...
/** Full catalog from tools.ts (190 tools across all modules) */
const CATALOG_TOOLS: ToolsPageItem[] = toolCategories.flatMap((cat) =>
  cat.tools.map((tool) => ({
    id: `${cat.slug}-${tool.slug}`,
    name: tool.name,
    ...
    trending: Boolean(tool.featured),
  }))
);

const BROWSE_CATEGORIES = toolCategories.map((cat) => ({
  name: cat.name,
  count: cat.tools.length,
  icon: cat.icon,
  slug: cat.slug,
  color: CATEGORY_COLORS[cat.slug] ?? OCEAN_BLUE,
  tools: cat.tools.slice(0, 3).map((t) => t.name),
}));

// ============ COMPONENTS ============

// Trending Card Component
function TrendingCard({ tool, index }: { tool: ToolsPageItem; index: number }) {
...
function ToolItem({ tool, index }: { tool: ToolsPageItem; index: number }) {
  const Icon = getToolIcon(tool.icon);
...
function CategoryCard({ category, index }: { category: typeof BROWSE_CATEGORIES[0]; index: number }) {
  const Icon = getToolIcon(category.icon);
    ...
    <section className="relative py-20 bg-gradient-to-br from-white via-slate-50 to-white overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            {PLATFORM_STATS.tools} Free Tools for{" "}
...
// Trending Section
function TrendingSection({ tools }: { tools: ToolsPageItem[] }) {
  return (
    <motion.section
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
...
// All Tools Section
function AllToolsSection({ tools, totalCount }: { tools: ToolsPageItem[]; totalCount: number }) {
  return (
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Showing {tools.length} of {totalCount} calculators in the catalog
            </p>
          </div>
        </div>
...
function SearchResultsSection({ tools, query }: { tools: ToolsPageItem[]; query: string }) {
  return (
    <section className="py-16 bg-white">
      <div className="container mx-auto px-4">
        <div className="mb-8">
    ...
    <section className="py-16 bg-slate-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-slate-900 mb-3">Browse by Module</h2>
          <p className="text-lg text-slate-600">
            {PLATFORM_STATS.modules} modules · {PLATFORM_STATS.tools} tools in the catalog
          </p>
        </div>
...
function SEOBlock() {
  const faqData = [
    ...
    },
    {
      ...
      answer:
        `Yes! All ${PLATFORM_STATS.tools} tools on Shiportrade are completely free to use with no registration required. We believe in making logistics calculations accessible to everyone in the trade industry. Our tools are funded by premium features and partnerships with industry service providers.`,
    },
    {
...
export default function ToolsPage() {
  ...
  const trendingTools = useMemo(
    () => CATALOG_TOOLS.filter((tool) => tool.trending).slice(0, 8),
    []
  );

  const filteredTools = useMemo(() => {
    const query = searchQuery.toLowerCase().trim();
    if (!query) return CATALOG_TOOLS;

    return CATALOG_TOOLS.filter(
      (tool) =>
      ...
      <AnimatePresence mode="wait">
        {showSearchResults ? (
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <AllToolsSection tools={filteredTools} totalCount={PLATFORM_STATS.tools} />
          </motion.div>
        )}
      </AnimatePresence>
